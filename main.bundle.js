...
/***/ (function(module, exports) {
	...
	$(document).ready(function () {
	  ...
	  $("#fav-btn").on('click', function () {
	    var api_key = 'pKyQaaomPtQVuLqe0GjkRg';
	    $.ajax({
	      type: 'GET',
	      url: 'https://damp-lowlands-57714.herokuapp.com/api/v1/favorites?api_key=' + api_key,
	      success: function success(result) {
	        currentForecast(result);
	        dailyForecast(result);
	        hourlyForecast(result);
	...
	function dailyForecast(result) {
	  ...
	  for (i = 0; i < dailyData.length; i++) {
	    var day = dailyData[i];
	    $(".day-time").append($('<div/>').attr("id", '' + i).text('' + getWeekday(day["time"])));
	    $(".day-icon").append($('<div/>').attr("id", '' + i).text('' + noHyphen(day["icon"])));
	    $(".day-rain-prob").append($('<div/>').attr("id", '' + i).text('' + toRainPercent(day["rain_prob"])));
	    $(".day-temp-high").append($('<div/>').attr("id", '' + i).text('' + toFahrenheit(day["temp_high"])));
	    $(".day-temp-low").append($('<div/>').attr("id", '' + i).text('' + toFahrenheit(day["temp_low"])));
	  }
	}
